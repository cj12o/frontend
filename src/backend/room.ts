...
const getRoomdetail = async (room_id: number) => {
  try {
    const resp = await axios.get(
      import.meta.env.VITE_ROOM_EPT + `?id=${room_id}`,
    );
    return resp?.data[0];
  } catch (e) {
    console.error(e);
...
const createRoom = async (
  ...
  try {
    ...
    const resp = await axios.post(import.meta.env.VITE_ROOM_EPT, data, {
      headers: {
...
const updateRoom = async (
  ...
  try {
    ...
    const resp = await axios.patch(import.meta.env.VITE_ROOM_EPT, data, {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Token ${localStorage.getItem("cookie")}`,
      },
