...
const login = async (
  ...
  try {
    ...
    const resp = await axios.post(
      import.meta.env.VITE_LOGIN_EPT,
      { email, password },
      {
    ...
    const error =
      e.response?.data?.errors?.non_field_errors?.[0] || "unknown error";
...
const signup = async (username: string, email: string, password: string) => {
  try {
    const resp = await axios.post(
      import.meta.env.VITE_SIGNUP_EPT,
...
const logout = async (dispatch: Dispatch<any>, visited_rooms: object) => {
  try {
    ...
    const resp = await axios.post(import.meta.env.VITE_LOGOUT_EPT, data, {
      headers: {
