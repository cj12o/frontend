...
import {
  setName as reducerSetName,
  setProfile as reducerSetProfile,
} from "../store/authSlice";

const getProfile = async (name: string) => {
  try {
    const resp = await axios.get(
      import.meta.env.VITE_PROFILE_EPT + `${name}/`,
      {
...
const updateProfile = async (
  file: File | null = null,
  delete_profile_pic: boolean = false,
  bio: string = "",
  email_id: string = "",
  username: string = "",
  ...
  try {
    ...
    const resp = await axios.patch(
      import.meta.env.VITE_PROFILE_EPT + `${localStorage.getItem("name")}/`,
      data,
      {
        headers: {
